...
const WORK_ITEMS: WorkItemData[] = [
  ...
  },
  {
    ...
    image: '/projects/nomad.webp',
    link: 'https://nomad.example.com',
    reverse: true,
  },
];

/* ── Cursor-based floating parallax for each image card ── */
...
function WorkItem({ item }: { item: WorkItemData }) {
  ...
  const [hasImage, setHasImage] = useState(!!item.image);
  ...
  return (
    <motion.div
      ref={ref}
      className={`work-item${isReverse ? ' work-item--reverse' : ''}`}
      style={{ opacity }}
    >
      <motion.div
        className="work-img"
      ...
      >
        ...
        <a
          ...
          className="work-img-link"
          aria-label={`Visit ${item.title}`}
        >
          <span className="work-img-link-icon">
      ...
      <motion.div className="work-info" style={{ x: infoX }}>
